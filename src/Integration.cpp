...
void readTunedIntegrationBeforeDedispersionConf(tunedIntegrationBeforeDedispersionConf &tunedConf, const std::string &confFilename)
{
    ...
    while ( ! confFile.eof() )
    {
        ...
        if ( tunedConf.count(deviceName) == 0 )
        {
            std::map< unsigned int, Integration::integrationConf * > * container = new std::map< unsigned int, Integration::integrationConf * >();

            container->insert(std::make_pair(integration, parameters));
            tunedConf.insert(std::make_pair(deviceName, container));
        }
        else
        {
            tunedConf.at(deviceName)->insert(std::make_pair(integration, parameters));
        }
    }
    confFile.close();
}
